...
uint32_t udisk_current_command_pos = 0;
uint32_t udisk_next_command_pos = 0;
inline void get_udisk_commands(void) {
  ...
  while (commands_in_queue < BUFSIZE && !udisk_eof && !stop_buffering) {
    ...
    if (udisk_eof || n == -1 || n == -USB_NOT_DETECTED
      ...
      if(udisk_current_command_pos == 0){
        udisk_current_command_pos = udisk.get_index();
      }
      udisk.udisk_queue_pos = udisk_current_command_pos;

      udisk_pos_queue[cmd_queue_index_w] = udisk.get_index();
      udisk_next_command_pos = udisk_pos_queue[cmd_queue_index_r];
      udisk_current_command_pos = udisk_next_command_pos;
